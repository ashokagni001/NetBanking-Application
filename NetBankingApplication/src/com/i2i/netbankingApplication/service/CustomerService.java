...
/**
 * <p>
 * When values comes from CustomerController. CustomerService operate passing
 * value's to CustomerDao based on requset's from CustomerController. It can
 * operate validation before send the value's in CustomerDao. It can be send the
 * value's to CustomerController from CustomerDao.
 * </p>
 * 
 * @author TEAM-2
 * 
...
public class CustomerService {
	...
	/**
	 * <p>
	 * customer object get from CustomerController. In this customer is check
	 * already have bank account.If account exit given the customer id,
	 * password. If they not account return some use full message. In this
	 * method we done customer id and password manual generation. If customer
	 * add successfully return customer id and password.
	 * 
	 * @param customer
	...
	public void getUser(Customer customer) throws DataBaseException, CustomerDataException {
		...
		if (StringUtil.isValidFormat(customer.getDob())) {
			throw new DataBaseException(
					"YOUR FORMAT" + customer.getDob() + "FORMAT MUST 1/05/2000.INSERT VALID DOB..!!");
		}
		...
		customerDao.insertUser(customer.getAccountNumber(),
				new Customer(customerId, customer.getName(), customerAge, customer.getDob(), customer.getGender(),
						customer.getMobileNumber(), customer.getEmail(), password, customer.getAccountNumber(),
						"ACTIVE"));
	...
	public List getMiniStatementByCustomerId(String customerId) throws DataBaseException {
		Customer customer = customerDao.retrieveCustomerById(customerId);
		if (customer != null) {
		...
		if (customerDao.retriveAllUserRole().size() == 0) {
			...
			for (UserRole userRole : customerDao.retriveAllUserRole()) {
				if ((userRole.getCustomer().getCustomerId().equals(customerId))
						&& (userRole.getRole().getRoleId().equals(roleId))) {
					return false;
				}
			}
			return true;
