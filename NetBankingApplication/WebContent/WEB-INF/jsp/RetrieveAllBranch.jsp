    ...
    <center>
        ...
        <c:if test="${branch != null}">
            ...
            <table class="table table-striped">
                ...
                <tr>
                    <td align="center">
                        <c:out value="${branch.getIFSCode()}" />
                    </td>
                    <td align="center">
                        <c:out value="${branch.getEmailId()}" />
                    </td>
                    <c:set value="${branch.getAddress()}" var="address" />
                    <td align="center"><a href="viewBranchAddress?addressId=<c:out value="${address.addressId}"/>" style="color: blue">VIEW</a>
                    </td>
                    <td><a href="deleteBranchById?ifsc=<c:out value="${branches.getIFSCode()}"/>" style="color: blue">Delete</a>
                    </td>
                </tr>
            </table>
        </c:if>
                ...
                <% int sno=1 ; %>
                    <c:forEach items="${branches}" var="branches">
                        <tr>
                            ...
                            <td>
                                <c:out value="${branches.getIFSCode()}" />
                            </td>
                            <td>
                                <c:out value="${branches.getEmailId()}" />
                            </td>
                            <c:set value="${branches.getAddress()}" var="address" />
                            <td><a href="viewBranchAddress?addressId=<c:out value=" ${address.addressId} "/>" style="color: blue">VIEW</a>
                            </td>
                            <td><a href="deleteBranchById?ifsc=<c:out value=" ${branches.getIFSCode()} "/>" style="color: blue">Delete</a>
                            </td>
                        </tr>
