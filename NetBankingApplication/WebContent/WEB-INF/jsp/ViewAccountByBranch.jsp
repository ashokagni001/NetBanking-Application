    ...
    <center>
        ...
        <form action="getAccount" method="get">
            <table>
                <tr>
                    <td>
                        <input type="text" name="ifsc" placeholder="IFSC" required="required">
                    </td>
                    <td>
                ...
                <% int sno=1 ; %>
                    <c:forEach items="${accounts}" var="accounts">
                        <tr>
                            ...
                            <td>
                                <c:out value="${accounts.getAccountNumber()}" />
                            </td>
                            <td>
                                <c:set value="${accounts.getCustomer()}" var="customer" />
                                <c:out value="${customer.customerId}" />
                            </td>
                            <td>
                                <c:out value="${accounts.getBalance()}" />
                            </td>
                            <td>
                                <c:out value="${accounts.getAccountType()}" />
                            </td>
                            <td align="center" height="30" width="100"><a href="getCustomer?customerId=<c:out value="${customer.customerId}"/>">VIEW</a>
                        </tr>
                        <% sno++; %>
                    </c:forEach>
            </table>
