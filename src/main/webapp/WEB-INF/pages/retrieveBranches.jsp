	...
	<c:if test="${branch != null}">
		<div class="col-md-12 sizetable">
			<table class="table table-striped table-bordered table-hover"
				...
				<tr>
						<td><c:set value="${branch.getIFSCode()}" var="IFSC" /> <a
							href="getAccounts?IFSCode=<c:out value="${IFSC}"/>"
							style="color: blue"><c:out value="${IFSC}" /></a></td>
						<td><c:out value="${branch.getEmailId()}" /></td>
						<td><c:out value="${branch.address.address}" /></td>
						<td><c:out value="${branch.address.city}" /></td>
						<td><c:out value="${branch.address.province}" /></td>
					</tr>
			</table>
		</div>
	</c:if>
			...
			<table class="table table-striped table-bordered table-hover"
				...
				<c:forEach items="${branches}" var="branches">
					<tr>
						...
						<td><c:set value="${branches.getIFSCode()}" var="IFSC" /> <a
							href="getAccounts?IFSCode=<c:out value="${IFSC}"/>"
							style="color: blue"><c:out value="${IFSC}" /></a></td>
